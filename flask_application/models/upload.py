...
class Upload(SolrMixin, CreatorMixin, db.Document):
    ...
    # this is a filename that is based on other application metadata to
    # standardize filenames for users
    structured_file_name = db.StringField(max_length=255, unique=True)
    # if the file is being saved to an alternative uploads location it can be
    # specified here
    ...
    def full_path(self):
        ...
        if self.file_path.strip()[0:1] == '/':
            # in the rare case that the file_path is absolute, we drop the
    ...
    def set_file(self, path):
        ...
        if os.path.exists(path):
            ...
            if app.config['UPLOADS_DIR'] in path:
                # file is already in uploads directory, so don't bother moving
                # it
                file_path = path
            else:
            ...
            self.file_path = os.path.join(
                *splitpath(file_path)[len(splitpath(app.config['UPLOADS_DIR'])):])
    ...
    def unique_structured_file_name(self, value=None, appendage=0):
        ...
        if value is None:
            value = orig_path
        to_slugify = "%s%s" % (
            value, ext) if appendage == 0 else "%s-%s%s" % (value, appendage, ext)
    ...
    def apply_calibre_folder_structure(self, data):
        ...
        """
        def safe_name(str):
            # return "".join([c for c in str if c.isalpha() or c.isdigit() or
            # c==' ']).rstrip()[:64]
        ...
        new_path = os.path.join(app.config['UPLOADS_DIR'], app.config[
                                'UPLOADS_SUBDIR'], directory1, directory2, filename)
        ...
        # rename to move
        try:
            new_dir = os.path.join(app.config['UPLOADS_DIR'], app.config[
            ...
            self.file_path = os.path.join(
                *splitpath(new_path)[len(splitpath(app.config['UPLOADS_DIR'])):])
    ...
    def recover_broken_file(self, data):
            ...
            for root, dirs, files in os.walk(path):
                for filename in files:
                    ...
                    if md5 == the_md5:
                        self.file_name = filename
                        self.file_path = os.path.join(
                            *splitpath(p)[len(splitpath(app.config['UPLOADS_DIR'])):])
        ...
        found = look_for_file(os.path.join(app.config['UPLOADS_DIR'], app.config[
                              'UPLOADS_SUBDIR'], directory1), self.md5)
    ...
    def plaintext(self):
        if self.md5 and 'TXT_SUBDIR' in app.config:
            txt_path = os.path.join(
                app.config['TXT_SUBDIR'], self.md5, "%s.txt" % self.md5)
    ...
    def build_solr(self):
        # I think this will cause major problems when uploads are saved because
        # it triggers a pdf text extraction
        return {}
    ...
    def ipfs_add(self):
        ...
        if os.path.exists(self.full_path()):
            ...
            raise Exception(
                "ipfs_add couldn't add non-existent file: %s" % (self.full_path(),))
