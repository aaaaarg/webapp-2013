...
def api_search(type=False):
    ...
    if type == 'things':
        results = elastic.search('thing',
                                 query={
                                     'title^3,short_description,description': query},
                                 num=num)
    elif type == 'makers':
    ...
    for id, score, result in results:
        ...
        if type == 'collections':
            retval['data'][id] = "%s (%s items)" % (
                result['title'], len(result['things']))
...
def research(filter_type=None, filter_id=None):
    ...
    if ready:
        ...
        if filter_type and filter_id:
            ...
            more = url_for('frontend.research', filter_type=filter_type,
                           filter_id=filter_id, query=query, page=page + 1)
        ...
        for md5, num_hits, top_hits in results:
            ...
            try:
                ...
                pass
        content = get_template_attribute(
            'frontend/macros.html', 'fulltext_search_results')(things, query)
...
def opensearch():
    ...
    etree.SubElement(root, 'Description').text = 'Search ' + \
...
def opensearch_search():
    ...
    results = elastic.search('thing',
                             query={
                                 'title^3,short_description,description,makers_string': q},
                             start=start,
                             num=num)
    ...
    feed = AtomFeed("Search results for '%s'" %
                    (q,), feed_url=request.url, url=request.url_root)
    for thing in things:

        # TODO: only send the last Upload per mimetype?
        links = [_create_link_dict(upload, request.host)
                 for upload in thing.files]
        ...
        feed.add(FeedEntry(
            ...
            author=authors,
            url="http://%s%s" % (request.host,
                                 url_for('thing.detail', id=thing.id),),
            links=links,
