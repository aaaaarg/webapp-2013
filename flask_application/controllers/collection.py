...
app.jinja_env.globals[
    'can_remove_thing_from_collection'] = can_remove_thing_from_collection
app.jinja_env.globals[
    'can_add_thing_to_collection'] = can_add_thing_to_collection
...
def list(letter=None):
    ...
    collections = Collection.objects.filter(
        title__istartswith=letter, accessibility__ne='private', supercollection__exists=False)
...
def edit(id):
    ...
    if not can_edit_collection(model):
        abort(403)
    form = CollectionForm(formdata=request.form,
                          obj=model, exclude=['creator'])
