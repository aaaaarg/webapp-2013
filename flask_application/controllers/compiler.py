...
def create_from_search():
    ...
    if not query == "":
        new_query = "%s" % query
        the_query = solr.query(searchable_text=new_query).filter(content_type="page").filter_exclude(
            md5_s="7dbf4aee8eb2b19197fe62913e15dda5").sort_by("-score").paginate(start=start, rows=num)
        ...
        # Go back through the grouped results to build the annotations to
        # return
        clips = []
        for md5 in sorted(grouped_results, key=lambda k: len(grouped_results[k]), reverse=True):
            ...
            if u:
                ...
                link = url_for("reference.figleaf", md5=md5,
                               _anchor='%s' % (sp[0]))
                img = url_for("reference.preview", filename=u.preview(
                    filename='%s-%sx%s.jpg' % (sp[0], sp[0], 75)))
                page_count = len(sp)
...
def build_clips(annotations):
    ...
    for a in annotations:
        if a.pos_end:
            ...
            img = url_for("reference.preview", filename=u.preview(
                filename='%s-%sx%s.jpg' % (y1, y1, 75)))
            #img = img.replace('/pages/','http://127.0.0.1:8484/')
            page_count = y2 - y1 + 1
            url_part = '%s.pdf/%s-%s/' % (a.upload.md5, y1,
                                          y2) if y2 > y1 else '%s.pdf/%s/' % (a.upload.md5, y1)
